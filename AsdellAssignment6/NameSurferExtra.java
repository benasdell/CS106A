 ...
 * 
 * NameSurferExtra:
 * This extension of NameSurfer adds some new functionality such as a delete button that 
 * allows the user to type in an already displayed name and remove only that name's graph
 * from the display instead of clearing all graphs at one time. Another addition for 
 * readability and more interesting display is visible data points of three different shapes
 * that match the color of their graph set.
 */
...
public class NameSurferExtra extends GraphicsProgram implements NameSurferConstants {
	...
	public void init() {
		...
		JButton deleteButton = new JButton("Delete"); //EXTRA: Delete button instance
	 ...
	 * the background. Delete functionality is also included by using the delete button
	 * after typing an already displayed name.
	 */
	public void actionPerformed(ActionEvent e) {
		...
		else if (e.getActionCommand().equals("Delete")) { //EXTRA: Delete functionality, removes name from ArrayList of previous displays
			NameSurferEntry deleteEntry = nameData.findEntry(name);
			if (deleteEntry != null) {
				dataCount = displayedNames.indexOf(name) + 1; //keeps remaining lines the same color
	...
	private void drawDataLine() {
		...
		for (int i = 0; i < NDECADES; i++) {
			...
			GObject pointMarker = shapeSelection(); //EXTRA: Added points to display to improve readability of graph
			dataLabel.setLocation(dataPoints[i]);
			pointMarker.setCenterLocation(dataPoints[i]);
	...
	/*
	 * EXTRA: This helper method first builds an array of various GObjects that are used to
	 * graph data points in the drawDataLine method. It uses similar logic as colorSelection
	 * to rotate through the array, although the colorSelection and shapeSelection methods have 
	 * different sized arrays and therefore will return different combinations of colors and shapes.
	 */
	private GObject shapeSelection() {
		GObject[] shapes = new GObject[3];
		GOval pointMarker = new GOval(4,4);
		pointMarker.setFilled(true);
		pointMarker.setColor(colorSelection());
		pointMarker.setFillColor(colorSelection());
		shapes[0] = pointMarker;
		GRect squareMarker = new GRect(4,4);
		squareMarker.setFilled(true);
		squareMarker.setColor(colorSelection());
		squareMarker.setFillColor(colorSelection());
		shapes[1] = squareMarker;
		GPolygon triangleMarker = new GPolygon();
		triangleMarker.addVertex(0, -2);
		triangleMarker.addVertex(-2, 2);
		triangleMarker.addVertex(2, 2);
		triangleMarker.setFilled(true);
		triangleMarker.setFillColor(colorSelection());
		shapes[2] = triangleMarker;
		return shapes[dataCount % 3];
	}
}
